...
static void
zgn_expansive_protocol_set_region(struct wl_client *client,
...
static void
zgnr_expansive_handle_virtual_object_commit(
    struct wl_listener *listener, void *data)
{
  UNUSED(data);
  struct zgnr_expansive_impl *self =
  ...
  if (self->pending.damage & ZGNR_EXPANSIVE_DAMAGE_REGION) {
