...
#define CURVE_MIN_Z (-0.08f)
#define CURVE_MAX_Z (+0.02f)
...
{
  if (-0.08 < z && z <= -0.04551448) {
    return sqrtf(powf(0.04, 2) - powf(z + 0.04, 2)) / 6.f;
  } else if (-0.04551448 <= z && z <= -0.015059523) {
    return -0.0001 / (z + 0.0039) + 0.0042;
  } else if (-0.015059523 <= z && z <= 0.02) {
    return sqrtf(powf(0.02, 2) - powf(z, 2));
  } else {
    return 0.f;
  }
}
