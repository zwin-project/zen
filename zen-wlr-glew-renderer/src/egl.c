...
egl_log_importance_to_wlr(EGLint type)
{
  switch (type) {
...
egl_log(EGLenum error, const char *command, EGLint msg_type,
    EGLLabelKHR thread UNUSED, EGLLabelKHR obj UNUSED, const char *msg)
{
...
static int get_egl_dmabuf_formats(struct zn_wlr_egl *egl, int **formats);
static int get_egl_dmabuf_modifiers(struct zn_wlr_egl *egl, EGLint format,
    uint64_t **modifiers, EGLBoolean **external_only);

static void
init_dmabuf_formats(struct zn_wlr_egl *egl)
{
  EGLint *formats = NULL;
  ...
  for (int i = 0; i < formats_len; i++) {
    ...
    wlr_drm_format_set_add(
        &egl->dmabuf_texture_formats, fmt, DRM_FORMAT_MOD_INVALID);
    wlr_drm_format_set_add(
        &egl->dmabuf_render_formats, fmt, DRM_FORMAT_MOD_INVALID);

    if (modifiers_len == 0) {
      // Asume the linear layout is supported if the driver doesn't
      // explicitly say otherwise
      wlr_drm_format_set_add(
          &egl->dmabuf_texture_formats, fmt, DRM_FORMAT_MOD_LINEAR);
      wlr_drm_format_set_add(
          &egl->dmabuf_render_formats, fmt, DRM_FORMAT_MOD_LINEAR);
    }
  ...
  for (int i = 0; i < formats_len; i++) {
    ...
    snprintf(&str_formats[(ptrdiff_t)i * 5], (formats_len - i) * 5 + 1, "%.4s ",
        (char *)&formats[i]);
  }
...
egl_create(void)
{
  ...
  struct zn_wlr_egl *egl = calloc(1, sizeof(struct zn_wlr_egl));
  if (egl == NULL) {
  ...
  if (check_egl_ext(client_exts_str, "EGL_EXT_device_base") ||
    ...
    load_egl_proc(
        &egl->procs.eglQueryDeviceStringEXT, "eglQueryDeviceStringEXT");
    load_egl_proc(
        &egl->procs.eglQueryDisplayAttribEXT, "eglQueryDisplayAttribEXT");
  }

  if (check_egl_ext(client_exts_str, "EGL_KHR_debug")) {
    load_egl_proc(
        &egl->procs.eglDebugMessageControlKHR, "eglDebugMessageControlKHR");
...
static bool
egl_init_display(struct zn_wlr_egl *egl, EGLDisplay *display)
{
  egl->display = display;
  ...
  if (check_egl_ext(
          display_exts_str, "EGL_EXT_image_dma_buf_import_modifiers")) {
    egl->exts.EXT_image_dma_buf_import_modifiers = true;
    load_egl_proc(
        &egl->procs.eglQueryDmaBufFormatsEXT, "eglQueryDmaBufFormatsEXT");
    load_egl_proc(
        &egl->procs.eglQueryDmaBufModifiersEXT, "eglQueryDmaBufModifiersEXT");
  }
  ...
  if (egl->exts.EXT_device_query) {
    ...
    if (!egl->procs.eglQueryDisplayAttribEXT(
            egl->display, EGL_DEVICE_EXT, &device_attrib)) {
    ...
    if (check_egl_ext(device_exts_str, "EGL_EXT_device_persistent_id")) {
      driver_name =
          egl->procs.eglQueryDeviceStringEXT(egl->device, EGL_DRIVER_NAME_EXT);
    }
#endif
...
static bool
egl_init(struct zn_wlr_egl *egl, EGLenum platform, void *remote_display)
{
  EGLDisplay display =
      egl->procs.eglGetPlatformDisplayEXT(platform, remote_display, NULL);
  ...
  egl->context = eglCreateContext(
      egl->display, EGL_NO_CONFIG_KHR, EGL_NO_CONTEXT, attribs);
  ...
  if (request_high_priority) {
    ...
    eglQueryContext(
        egl->display, egl->context, EGL_CONTEXT_PRIORITY_LEVEL_IMG, &priority);
...
static EGLDeviceEXT
get_egl_device_from_drm_fd(struct zn_wlr_egl *egl, int drm_fd)
{
  ...
  for (int i = 0; i < nb_devices; i++) {
    const char *egl_device_name =
        // NOLINTNEXTLINE(clang-analyzer-core.CallAndMessage)
        egl->procs.eglQueryDeviceStringEXT(devices[i], EGL_DRM_DEVICE_FILE_EXT);
...
struct zn_wlr_egl *
zn_wlr_egl_create_with_drm_fd(int drm_fd)
{
  struct zn_wlr_egl *egl = egl_create();
  if (egl == NULL) {
    wlr_log(WLR_ERROR, "Failed to create EGL context");
    return NULL;
  }
...
struct zn_wlr_egl *
zn_wlr_egl_create_with_context(EGLDisplay display, EGLContext context)
{
  EGLint client_type = 0;
  if (!eglQueryContext(
          display, context, EGL_CONTEXT_CLIENT_TYPE, &client_type) ||
  ...
  if (!eglQueryContext(
          display, context, EGL_CONTEXT_CLIENT_VERSION, &client_version) ||
      client_version < 2) {
    wlr_log(WLR_ERROR,
        "Unsupported EGL context client version (need OpenGL ES >= 2)");
    return NULL;
  }
...
void
zn_wlr_egl_destroy(struct zn_wlr_egl *egl)
{
  if (egl == NULL) {
    return;
  }
...
bool
zn_wlr_egl_destroy_image(struct zn_wlr_egl *egl, EGLImage image)
{
...
bool
zn_wlr_egl_make_current(struct zn_wlr_egl *egl)
{
  if (!eglMakeCurrent(
          egl->display, EGL_NO_SURFACE, EGL_NO_SURFACE, egl->context)) {
...
bool
zn_wlr_egl_unset_current(struct zn_wlr_egl *egl)
{
  if (!eglMakeCurrent(
          egl->display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT)) {
    wlr_log(WLR_ERROR, "eglMakeCurrent failed");
...
bool
zn_wlr_egl_is_current(struct zn_wlr_egl *egl)
{
  return eglGetCurrentContext() == egl->context;
}

void
zn_wlr_egl_save_context(struct zn_wlr_egl_context *context)
{
...
bool
zn_wlr_egl_restore_context(struct zn_wlr_egl_context *context)
{
  ...
  return eglMakeCurrent(
      display, context->draw_surface, context->read_surface, context->context);
}

EGLImageKHR
zn_wlr_egl_create_image_from_dmabuf(struct zn_wlr_egl *egl,
    struct wlr_dmabuf_attributes *attributes, bool *external_only)
{
  ...
  attribs[atti++] = attributes->format;  // NOLINT(*-narrowing-conversions)
  ...
  for (int i = 0; i < attributes->n_planes; i++) {
    ...
    attribs[atti++] = attributes->offset[i];  // NOLINT(*-narrowing-conversions)
    attribs[atti++] = attr_names[i].pitch;
    attribs[atti++] = attributes->stride[i];  // NOLINT(*-narrowing-conversions)
  ...
  EGLImageKHR image = egl->procs.eglCreateImageKHR(
      egl->display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, attribs);
  if (image == EGL_NO_IMAGE_KHR) {
    wlr_log(WLR_ERROR, "eglCreateImageKHR failed");
    return EGL_NO_IMAGE_KHR;
  }

  *external_only = !wlr_drm_format_set_has(
      &egl->dmabuf_render_formats, attributes->format, attributes->modifier);
  return image;
}

static int
get_egl_dmabuf_formats(struct zn_wlr_egl *egl, int **formats)
{
...
static int
get_egl_dmabuf_modifiers(struct zn_wlr_egl *egl, EGLint format,
    uint64_t **modifiers, EGLBoolean **external_only)
{
  *modifiers = NULL;
  *external_only = NULL;
  ...
  if (!egl->procs.eglQueryDmaBufModifiersEXT(
          egl->display, format, 0, NULL, NULL, &num)) {
...
zn_wlr_egl_get_dmabuf_texture_formats(struct zn_wlr_egl *egl)
{
  return &egl->dmabuf_texture_formats;
}

const struct wlr_drm_format_set *
zn_wlr_egl_get_dmabuf_render_formats(struct zn_wlr_egl *egl)
{
...
int
zn_wlr_egl_dup_drm_fd(struct zn_wlr_egl *egl)
{
  ...
  if (egl->exts.EXT_device_drm_render_node) {
    const char *name = egl->procs.eglQueryDeviceStringEXT(
        egl->device, EGL_DRM_RENDER_NODE_FILE_EXT);
    if (name == NULL) {
  ...
  if (render_name == NULL) {
    ...
    if (primary_name == NULL) {
      wlr_log(
          WLR_ERROR, "eglQueryDeviceStringEXT(EGL_DRM_DEVICE_FILE_EXT) failed");
      return -1;
    }
    ...
    if (render_name == NULL) {
      wlr_log(
          WLR_ERROR, "Can't find render node name for device %s", primary_name);
      return -1;
    }
  }
