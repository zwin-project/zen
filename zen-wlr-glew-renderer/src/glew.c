...
glew_begin(struct wlr_renderer *wlr_renderer, uint32_t width, uint32_t height)
{
  ...
  glViewport(0, 0, width, height);  // NOLINT(*-narrowing-conversions)
  renderer->viewport_width = width;
  renderer->viewport_height = height;

  // refresh projection matrix
  wlr_matrix_projection(  // NOLINTNEXTLINE(*-narrowing-conversions)
    ...
    uint32_t src_x, uint32_t src_y, uint32_t dst_x, uint32_t dst_y, void *data)
{
  ...
  unsigned char *p = (unsigned char *)data + (size_t)dst_y * stride;
...
glew_destroy(struct wlr_renderer *wlr_renderer)
{
  ...
  struct wlr_glew_buffer *buffer = NULL;
  struct wlr_glew_buffer *buffer_tmp = NULL;
  wl_list_for_each_safe (buffer, buffer_tmp, &renderer->buffers, link) {
    destroy_buffer(buffer);
  }

  struct wlr_glew_texture *tex = NULL;
  struct wlr_glew_texture *tex_tmp = NULL;
    ...
    struct wlr_glew_renderer *renderer, const char *file, const char *func)
{
  ...
  char str[len];
  snprintf(str, len, "%s:%s", file, func);  // NOLINT(cert-err33-c)
