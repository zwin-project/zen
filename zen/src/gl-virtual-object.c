...
zn_gl_virtual_object_create(void *impl_data,
    const struct zn_gl_virtual_object_interface *implementation,
    struct zn_xr_dispatcher *dispatcher)
{
  struct zn_gl_virtual_object *self = zalloc(sizeof *self);
  if (self == NULL) {
...
void
zn_gl_virtual_object_destroy(struct zn_gl_virtual_object *self)
{
