...
static void zn_xdg_popup_view_destroy(struct zn_xdg_popup_view* self);

static void
zn_xdg_popup_view_map(struct wl_listener* listener, void* data)
{
  struct zn_xdg_popup_view* self =
...
static void
zn_xdg_popup_view_unmap(struct wl_listener* listener, void* data)
{
  struct zn_xdg_popup_view* self =
  ...
  if (!zn_assert(
          zn_view_is_mapped(&self->base), "Tried to unmap a unmapped view"))
    return;
...
static void
zn_xdg_popup_view_wlr_xdg_surface_destroy_handler(
    struct wl_listener* listener, void* data)
{
  ...
  zn_xdg_popup_view_destroy(self);
}

static struct wlr_surface*
zn_xdg_popup_view_impl_get_wlr_surface(struct zn_view* view)
{
  struct zn_xdg_popup_view* self = zn_container_of(view, self, base);
  return self->wlr_xdg_popup->base->surface;
}

static const struct zn_view_impl zn_xdg_popup_view_impl = {
    .get_wlr_surface = zn_xdg_popup_view_impl_get_wlr_surface,
};

struct zn_xdg_popup_view*
zn_xdg_popup_view_create(
    struct wlr_xdg_popup* wlr_xdg_popup, struct zn_server* server)
{
  ...
  zn_view_init(&self->base, ZN_VIEW_XDG_POPUP, &zn_xdg_popup_view_impl);

  self->wlr_xdg_popup = wlr_xdg_popup;

  self->map_listener.notify = zn_xdg_popup_view_map;
  wl_signal_add(&self->wlr_xdg_popup->base->events.map, &self->map_listener);

  self->unmap_listener.notify = zn_xdg_popup_view_unmap;
  wl_signal_add(
...
static void
zn_xdg_popup_view_destroy(struct zn_xdg_popup_view* self)
{
