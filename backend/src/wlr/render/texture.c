    ...
    uint32_t dst_x, uint32_t dst_y, const void *data)
{
  ...
  zn_wlr_egl_make_current(texture->renderer->egl);
...
glew_texture_destroy(struct wlr_glew_texture *texture)
{
  ...
  zn_wlr_egl_destroy_image(texture->renderer->egl, texture->image);
    ...
    struct wlr_renderer *wlr_renderer, struct wlr_dmabuf_attributes *attribs)
{
  ...
  texture->image = zn_wlr_egl_create_image_from_dmabuf(
      renderer->egl, attribs, &external_only);
  if (texture->image == EGL_NO_IMAGE_KHR) {
    ...
    struct wlr_renderer *wlr_renderer, struct wlr_buffer *buffer)
{
  ...
  if (wlr_buffer_begin_data_ptr_access(
          buffer, WLR_BUFFER_DATA_PTR_ACCESS_READ, &data, &format, &stride)) {
